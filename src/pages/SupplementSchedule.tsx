...
import { HEALTH_TAG_LABELS, HEALTH_TAG_COLORS, SUPPLEMENT_TIMING_LABELS, type HealthTag, type SupplementTiming } from "../data/types";

type FilterType = "all" | "supplement";
          ...
          <div key={item.id} className="py-2 border-b border-slate-800/50 cursor-pointer" onClick={() => onToggle(item.id)}>
            <div className="flex justify-between items-start">
              <div className="flex-1">
                <div className="flex items-center gap-1 flex-wrap">
                  ...
                  {item.isActive && <span className="text-[9px] font-bold text-red-400 bg-red-900/30 px-1 py-0.5 rounded">★核心</span>}
                </div>
                <div className="text-xs text-slate-500 mt-0.5">{item.dosagePerUnit}</div>
            ...
            {open && (
              <div className="mt-2 pl-2 border-l-2 border-slate-700 text-xs space-y-1">
                {item.mechanism && <p className="text-slate-300">{item.mechanism}</p>}
                {item.timing && item.timing.length > 0 && <p className="text-blue-400/70">⏰ {(item.timing as SupplementTiming[]).map((t: SupplementTiming) => SUPPLEMENT_TIMING_LABELS[t]).join("、")}</p>}
